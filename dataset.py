...
import mlconfig
import pandas
from util import onehot, rand_bbox, get_model
...
transform_options['PoisonAgnosticCIFAR10Folder'] = transform_options['AgnosticCIFAR10Folder']
...
class DatasetGenerator():
    def __init__(self, train_batch_size=128, eval_batch_size=256, num_of_workers=4,
        ...
        elif train_data_type == 'AgnosticCIFAR10Folder':
            num_of_classes = 10
            train_dataset = AgnosticCIFAR10Folder(root=train_data_path, transform=train_transform,
                                                  total_config=self.total_config)
        elif train_data_type == 'PoisonAgnosticCIFAR10Folder':
            num_of_classes = 10
            train_dataset = PoisonAgnosticCIFAR10Folder(root=train_data_path, transform=train_transform,
                                                        poison_rate=poison_rate, perturb_type=perturb_type,
                                                        patch_location=patch_location, seed=seed,
...
class PoisonCIFAR10(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 ...
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None):
        super(PoisonCIFAR10, self).__init__(root=root, train=train, download=download, transform=transform,
                                            target_transform=target_transform)
        self.perturb_tensor = torch.load(perturb_tensor_filepath, map_location=device)
        # self.perturb_tensor = torch.zeros_like(self.perturb_tensor)
...
class PoisonCIFAR100(datasets.CIFAR100):
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        super(PoisonCIFAR100, self).__init__(root=root, train=train, download=download, transform=transform,
                                             target_transform=target_transform)
        self.perturb_tensor = torch.load(perturb_tensor_filepath, map_location=device)
        self.perturb_tensor = self.perturb_tensor.mul(255).clamp_(0, 255).permute(0, 2, 3, 1).to('cpu').numpy()
        self.patch_location = patch_location
...
class PoisonSVHN(datasets.SVHN):
    def __init__(self, root, split='train', transform=None, target_transform=None,
                 ...
                 seed=0, perturb_type='classwise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False):
        super(PoisonSVHN, self).__init__(root=root, split=split, download=download, transform=transform,
                                         target_transform=target_transform)
        self.perturb_tensor = torch.load(perturb_tensor_filepath, map_location=device)
...
class AgnosticCIFAR10Folder(ImageFolder):
    def __init__(self, root, transform, random_sort=True, total_config=None, drop_portion=False):
        super(AgnosticCIFAR10Folder, self).__init__(root=root, transform=transform)

        # ImageFolder直接读取会把一类样本读取到一起，当loader接口中的shuffle=False时无法进行训练，故在此生成order属性打乱
        if random_sort:
            self.order = np.random.permutation(50000).tolist()
        else:
            self.order = list(range(50000))
        if 'drop_portion' in total_config:
            self.drop_portion = total_config.drop_portion
            self.primary_idx = self.order[:int(len(self.order) * (1 - self.drop_portion))]
        else:
            self.primary_idx=self.order

        # 加载部分数据

        self.data, self.targets = self._load_data(self.primary_idx)

        # 利用聚类模型生成聚类标签
        if 'clustering_model' in total_config:
            self.clustering_model_config = total_config.clustering_model_config

            print('{} Clustering targets generated ... ')
            self.clustering_model = self._load_clustering_model(self.clustering_model_config)
            self.cluster_targets = self.clustering_model(self.data)

    def _load_data(self, load_list):
        # 按照伪造标签投毒
        l_data = []
        l_targets = []
        i = 0
        print(f'Converge to self.data...')
        for idx in load_list:
            ...
            img = Image.open(path)
            img_np = np.array(img)
            l_data.append(img_np)
            l_targets.append(target)
            img.close()
            if (i) % 1000 == 0:
                print(f'\r{i // 1000}k samples converged.', end='')
        print()
        l_data = np.stack(l_data, axis=0)
        return l_data, l_targets
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'clustering_model_config'):
            return sample, self.clustering_model(target)
        return sample, target
    ...
    # 加载聚类模型
    def _load_clustering_model(self, config):
        from models.ClusteringModel import OneHeadClusteringModel
        clustering_model_kwargs = {}

        train_model_transformation = transforms.Compose(
            [transforms.Normalize(mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])])
        # pretrained scan model trained on clean data
        # get_util() fun coded in util.py, OneHeadClusteringModel class coded in models
        pretrained_model = get_model(config, transforms=train_model_transformation)
        pretrained_selflabel_dict = torch.load(config.clustering_model_path)
        pretrained_model.load_state_dict(pretrained_selflabel_dict['model'])
        pretrained_one_head_model = OneHeadClusteringModel(pretrained_model).cuda().eval()

        return pretrained_one_head_model


class PoisonAgnosticCIFAR10Folder(AgnosticCIFAR10Folder):
    def __init__(self, root, transform, poison_rate=1.0, perturb_tensor_filepath=None,
                 seed=0, perturb_type='classwise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None,
                 total_config=None):
        super(PoisonAgnosticCIFAR10Folder, self).__init__(root=root, transform=transform,
                                                          random_sort=True,
        ...
        # 加入第二部分数据
        if len(self.order)!=len(self.primary_idx):
            self.add_post_data()

        # 原来打的标签有一部分是错的，把标签改回来
        self.label_traceback()

    def add_post_data(self):

        self.post_idx = [i for i in self.order if i not in self.primary_idx]

        i = 0
        print('Add post data ... ')
        post_data, post_targets = self._load_data(self.post_idx)
        self.data = np.concatenate([self.data, post_data], axis=0)
        self.targets.extend(post_targets)
        print('Add post data finished .')

    def label_traceback(self):
        num = 0
        err_labels = 0
        for ordered_idx, i in enumerate(self.order):
            num += 1
            # imgs属性中的图片地址是按照原来顺序排列的
            base_name = os.path.basename(self.imgs[i][0])
            # 文件以 agnostic_label_str_true_label_str_i 命名
            agnostic_label_str, true_label_str = base_name.split('_')[0], base_name.split('_')[1]
            if agnostic_label_str != true_label_str:
                err_labels += 1
                self.targets[ordered_idx] = self.class_to_idx[true_label_str]
            if num % 1000 == 0:
                print(f'{num // 1000} k samples trace back. {err_labels} samples tagged false. ')
