...
class Trainer():
    ...
    def train_batch(self, images, labels, model, optimizer):
        model.zero_grad()
        optimizer.zero_grad()
        if isinstance(self.criterion, torch.nn.CrossEntropyLoss) or \
                isinstance(self.criterion, models.CutMixCrossEntropyLoss) or \
                isinstance(self.criterion,losses.SCELoss):

            logits = model(images)
            loss = self.criterion(logits, labels)

        elif isinstance(self.criterion, losses.MeanAbsoluteError):
            logits = model(images)
